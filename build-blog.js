...
const blogDir = path.resolve('dist');
fsExtra.ensureDirSync(blogDir);

// Create blog posts directory
const postsDir = path.resolve('blog');
...
function processBlogPosts() {
  ...
  // Read the CSV file
  const csvData = fs.readFileSync('blogposts.csv', 'utf8');
...
<body>
  ...
  records.forEach((record, index) => {
    ...
    let processedContent = content.replace(/(\s+)(#{2,4}\s+[\w\s\-:&',]+)(\s+)/g, "\n\n$2\n\n");
...
<body>
    ...
    // Save the post HTML
    fs.writeFileSync(`${postsDir}/${slug}.html`, postContent);
  ...
  fs.writeFileSync(`${postsDir}/index.html`, blogListingContent);
